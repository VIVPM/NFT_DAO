...
import { useGlobalState, setGlobalState, truncate, setAlert } from '../store1'
import { buyNFT } from '../Blockchain.Services1'
...
const ShowNFT = () => {
  ...
  // const onContribute(uint256 amount) = async () => {
  ...
  const handleNFTPurchase = async () => {
    ...
    try {
      await buyNFT(nft)
      await performContribute(nft.cost)
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-screen h-screen flex items-center
          justify-center bg-blue bg-opacity-50 transform
          transition-transform duration-300 ${showModal}`}
    >
      <div className="bg-[#075985] shadow-xl shadow-[#e32970] rounded-xl w-11/12 md:w-2/5 h-7/12 p-6">
          ...
          <div className="flex justify-between items-center space-x-2">
            {connectedAccount == nft?.owner ? (
              <button
                ...
                w-full text-[#e32970] text-md border-[#075985]
